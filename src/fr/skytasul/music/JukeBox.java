...
public class JukeBox extends JavaPlugin implements Listener {
    ...
    public void onLocalData(final LocalDataLoadEvent e) {
        ...
        final PlayerData pd = new PlayerData(p, defaultPlayer);//PlayerData.create(id);//PlayerData.players.get(id);
        PlayerData.players.put(id, pd);
        if (JukeBox.autoJoin) {
            pd.setJoinMusic(true);
        } else if (p.hasPermission("ostrov.music")) {
            if (pd.songPlayer == null) {
                if (pd.hasJoinMusic()) {
                    pd.playRandom();
                }
            }
    ...
    public void onQuit(final PlayerQuitEvent e) {
        final PlayerData pd = PlayerData.players.remove(e.getPlayer().getUniqueId());
        if (pd.songPlayer != null) {
            pd.stopPlaying(false);
        }
    }
    ...
    public void onSongDestroy(final SongDestroyingEvent e) {
        ...
        for (UUID uuid : e.getSongPlayer().getPlayerUUIDs()) {
            pd = PlayerData.players.get(uuid);
            if (pd.linked!= null) {
                pd.linked.playingStopped();
            }
            pd.songPlayer = null;
            if (pd.favoritesRemoved) {
                pd.favoritesRemoved = false;
                if (pd.listening == Playlists.FAVORITES && pd.favorites != null) {
                    pd.playList(pd.favorites);
                }
            }
        }
    }
    ...
    public void onSongNext(final SongNextEvent e) {
        ...
        for (UUID uuid : e.getSongPlayer().getPlayerUUIDs()) {
            pd = PlayerData.players.get(uuid);
            if (pd.listening == Playlists.PLAYLIST && !pd.shuffle) {
                pd.stopPlaying(false);
            } else {
                pd.playSong(true);
            }
